...
describe('StatusSummary', () => {
  it('renders nothing when all counts are zero', () => {
    const { container } = render(<StatusSummary counts={{ open: 0, in_progress: 0, done: 0 }} />);
    expect(container).toBeEmptyDOMElement();
  });

  it('shows the three labels with counts when all statuses are present', () => {
    render(<StatusSummary counts={{ open: 2, in_progress: 1, done: 4 }} />);
  ...
  it('omits status segments with count 0', () => {
    render(<StatusSummary counts={{ open: 0, in_progress: 2, done: 1 }} />);
